## Edited by Bytez
## https://github.com/bytezz/godot-virtual_joystick
## ----
## Powerful virtual joystick for touchscreens
...
# Handle texture
export(Texture) var handleTexture

# Background texture
export(Texture) var backTexture

export var changeColorOnPress = true
...
# If true, the joystick will be hidden if not in use
export var hideNotInUse = false
...
export(String) var action_up = "vjs_up"
export(String) var action_right = "vjs_right"
export(String) var action_down = "vjs_down"
export(String) var action_left = "vjs_left"
var input_actions
...
func _ready() -> void:
	input_actions = [action_up,action_right,action_down,action_left]
	
	if not OS.has_touchscreen_ui_hint() and visibility_mode == VisibilityMode.TOUCHSCREEN_ONLY:
		hide()
	
	if handleTexture != null:
		_handle.set_texture(handleTexture)
	if backTexture != null:
		_background.set_texture(backTexture)
